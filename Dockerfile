...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y curl git zip unzip apache2 php{phpver} php{phpver}-curl php{phpver}-mbstring php{phpver}-xml php{phpver}-intl php{phpver}-zip php{phpver}-gd php{phpver}-soap php{phpver}-bcmath php{phpver}-imagick php{phpver}-mysql libapache2-mod-php{phpver} && apt-get clean && a2enmod php{phpver} && a2enmod rewrite && a2enmod ssl && sed -i "s/short_open_tag = Off/short_open_tag = On/" /etc/php/{phpver}/apache2/php.ini && sed -i "s/error_reporting = .*$/error_reporting = E_ERROR | E_WARNING | E_PARSE/" /etc/php/{phpver}/apache2/php.ini
